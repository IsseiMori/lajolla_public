...
struct register_embree_op {
    ...
    uint32_t operator()(const BezierCurve &curve) const;
...
struct sample_point_on_shape_op {
    ...
    PointAndNormal operator()(const BezierCurve &curve) const;
...
struct surface_area_op {
    ...
    Real operator()(const BezierCurve &curve) const;
};
...
struct init_sampling_dist_op {
    ...
    void operator()(BezierCurve &curve) const;
};
...
struct compute_shading_info_op {
    ...
    ShadingInfo operator()(const BezierCurve &curve) const;
...
#include "shapes/bezier_curve.inl"
